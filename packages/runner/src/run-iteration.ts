 ...
 *
 * Budget gating: if `deps.budget.tripped` is true the per-symbol loop is skipped
 * entirely and `state.lastTickedMs` is NOT advanced (matching paper-runner parity).
 * Callers must supply `deps.budget`; paper-runner's `BudgetTracker` satisfies the
 * shape structurally.
 */
