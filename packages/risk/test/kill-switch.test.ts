...
describe("KillSwitch", () => {
  ...
  it("auto-trips on daily DD exceeded", () => {
    const ks = new KillSwitch();
    ks.observe(
      { dailyPnlPct: -3.5, totalDdPct: -2, consecutiveLosses: 0, lastFeedAgeSec: 1 },
      {
        maxDailyLossPct: 3,
  ...
  it("trips on total drawdown exceeded", () => {
    const ks = new KillSwitch();
    ks.observe(
      { dailyPnlPct: 0, totalDdPct: -10, consecutiveLosses: 0, lastFeedAgeSec: 1 },
      {
        maxDailyLossPct: 3,
        maxTotalDrawdownPct: 8,
        maxConsecutiveLosses: 4,
  ...
  it("trips on consecutive losses exceeded", () => {
    const ks = new KillSwitch();
    ks.observe(
      { dailyPnlPct: 0, totalDdPct: 0, consecutiveLosses: 5, lastFeedAgeSec: 1 },
      {
        maxDailyLossPct: 3,
        maxTotalDrawdownPct: 8,
        maxConsecutiveLosses: 4,
  ...
  it("trips on stale feed", () => {
    const ks = new KillSwitch();
    ks.observe(
      { dailyPnlPct: 0, totalDdPct: 0, consecutiveLosses: 0, lastFeedAgeSec: 60 },
      {
        maxDailyLossPct: 3,
        maxTotalDrawdownPct: 8,
        maxConsecutiveLosses: 4,
  ...
  it("requires explicit reset()", () => {
    const ks = new KillSwitch();
    ks.observe(
      { dailyPnlPct: -10, totalDdPct: 0, consecutiveLosses: 0, lastFeedAgeSec: 1 },
      {
        maxDailyLossPct: 3,
        maxTotalDrawdownPct: 8,
        maxConsecutiveLosses: 4,
    ...
    expect(ks.state().tripped).toBe(true);
    ks.observe(
      { dailyPnlPct: 0, totalDdPct: 0, consecutiveLosses: 0, lastFeedAgeSec: 1 },
      {
        maxDailyLossPct: 3,
        maxTotalDrawdownPct: 8,
        maxConsecutiveLosses: 4,
